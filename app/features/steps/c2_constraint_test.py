...
def step_impl(context):
    context.dpop_to_test.room.set_devices([Device(1, 29, False)])
    context.dpop_to_test.room.tau = 60
...
def step_impl(context):
    ...
    assert_that(context.dpop_to_test.room.current_v, less_than_or_equal_to(29))


@when("an IoT device is in critical state and ends its program in less then 30 minutes")
def step_impl(context):
    context.dpop_to_test.room.set_devices([Device(1, 30, False), Device(2, 241, True)])
