...
def step_impl(context):
    set_up(context)
    context.current_dpop_tested.util_manager.JOIN = context.util_matrix


@then('should select the optimal assignment')
def step_impl(context):
    index = context.current_dpop_tested.value_manager.get_index_of_best_value_with(
        context.data,
        context.current_dpop_tested.util_manager.matrix_dimensions_order,
        context.current_dpop_tested.util_manager.JOIN
    )
...
def step_impl(context):
    assert_that(calling(context.current_dpop_tested.value_manager.get_index_of_best_value_with)
...
def step_impl(context):
    set_up(context)
    context.current_dpop_tested.util_manager.JOIN = numpy.asmatrix([[1], [0], [1]])
...
def set_up(context):
    context.current_dpop_tested.util_manager.matrix_dimensions_order = [1]
