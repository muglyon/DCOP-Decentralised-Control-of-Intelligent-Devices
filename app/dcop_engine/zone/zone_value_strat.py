...
class ZoneValueStrat(ValueStratAbstract):
    ...
    def do_value_propagation(self, join_matrix, util_matrix, matrix_dimensions_order):
        ...
        if util_matrix is None:
            util_matrix = numpy.zeros(DIMENSION_SIZE, int)

        if not self.dfs_structure.is_root:
            self.mqtt_manager.publish_util_msg_to(
                self.dfs_structure.parent_id,
                json.dumps({VARS: matrix_dimensions_order, DATA: util_matrix.tolist()})
            )
        ...
        self.dfs_structure.monitored_area.current_v = DIMENSION[index]
