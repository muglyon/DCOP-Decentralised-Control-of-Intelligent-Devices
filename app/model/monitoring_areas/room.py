...
class Room(MonitoringArea):
    ...
    def __init__(self, id):
        ...
        for device_id in range(0, randint(0, c.MAX_NB_DEVICES)):
    ...
    def add_or_update_device(self):
        ...
        self.device_list.append(
            Device(int(id_device), randint(c.MIN_TAU_VALUE, c.INFINITY), critic_state)
        )
    ...
    def pop_or_reprogram_devices(self):
        ...
        for device in self.device_list:
            ...
            if device.is_in_critic_state and random_number < 0.2:
                log.info("healthcare pro pop critical devices", self.id, c.EVENT)
                self.device_list.pop(self.device_list.index(device))
                continue

            log.info("healthcare pro reboot devices", self.id, c.EVENT)
            device.is_in_critic_state = False
            device.end_of_prog = c.INFINITY
